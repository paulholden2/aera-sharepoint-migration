...
class Application {
  ...
  run() {
    ...
    fs.readdir(baseDir, (err, files) => {
      ...
      const dirs = files.map(file => path.join(baseDir, file)).filter(file => {
        ...
        return stat.isDirectory() && file.endsWith(this.options.deliveryTriggerSuffix);
      });
      async.eachSeries(dirs.slice(0, 3), (dir, callback) => {
  ...
  async createApiDocumentSet(field, well) {
    ...
    let wellFolder;
    try {
      wellFolder = await this.sharepoint.getFolder(`/${field}/${api}`);
    ...
    if (!wellFolder) {
      wellFolder = await this.sharepoint.createListItem(field, api, wellInfoContentType[0].StringId);
  ...
  processDeliveryDir(dir) {
    ...
    if (!dir.endsWith(this.options.deliveryTriggerSuffix)) {
    ...
    fs.readdir(dir, (err, contents) => {
      ...
      this.createFieldApiSets(data).then(resources => {
        ...
        if (!this.options.skipUploads && !hadErrors) {
  ...
  async processDeliveryEntry(dir, entry) {
    ...
    const filePathNoDeliverySuffix = path.join(dir.slice(0, -this.options.deliveryTriggerSuffix.length), entry.Filename);
